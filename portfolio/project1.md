...
Using `mean_absolute_error`, we find we are (on average) 6.04 degrees Farenheit from the actual values; of course, this result isn't great, but predicting weather is difficult (especially Midwest weather). So, let's evaluate our model, and let's see if we can make some improvements.
